import json
import os
from pathlib import PurePosixPath

import audio_processing
import config
import main as ingest_main
import video_processing
from datasets import load_dataset
from hf_auth import hf_login

JSON_INPUT_FOLDER = os.path.join("data", "vista400k", "jsons")
VIDEO_OUTPUT_FOLDER = os.path.join("data", "vista400k", "videos")


def resolve_dataset_name(dataset_name=None):
    if dataset_name:
        return dataset_name
    try:
        import config as app_config

        configured_name = getattr(app_config, "DATASET_NAME", None)
        if configured_name:
            return configured_name
    except Exception:
        pass
    print("No dataset found in config.")
    return None


def get_entry_target_filename(entry):
    target_filename = entry.get("image") or entry.get("video") or entry.get("video_path")
    if not target_filename:
        return None
    target_filename = str(target_filename)
    if not target_filename.endswith(".mp4"):
        return None
    return target_filename


def build_targets(entries):
    targets = {}
    for entry in entries:
        target_filename = get_entry_target_filename(entry)
        if not target_filename:
            continue
        target_key = os.path.splitext(os.path.basename(target_filename))[0]
        targets[target_key] = target_filename
    return targets


def save_video_bytes(relative_path, video_bytes):
    local_output_path = os.path.join(VIDEO_OUTPUT_FOLDER, *PurePosixPath(relative_path).parts)
    os.makedirs(os.path.dirname(local_output_path), exist_ok=True)
    with open(local_output_path, "wb") as f:
        f.write(video_bytes)
    return local_output_path


def download_subset_videos(data_dir, entries, max_search=None, dataset_name=None):
    """
    Streams one dataset subset and downloads all target videos found in entries.
    Returns a dict: {target_filename: local_output_path}
    """
    dataset_name = resolve_dataset_name(dataset_name)
    if not dataset_name:
        return {}

    targets = build_targets(entries)
    if not targets:
        return {}

    print(f"--- Searching subset {data_dir}: {len(targets)} target(s) ---")
    found_paths = {}
    remaining_keys = set(targets.keys())

    try:
        hf_login()
        dataset = load_dataset(dataset_name, data_dir=data_dir, streaming=True, split="train")

        for i, dataset_entry in enumerate(dataset, 1):
            current_key = dataset_entry.get("__key__")
            if current_key not in remaining_keys:
                if max_search is not None and i >= max_search:
                    break
                continue

            video_bytes = dataset_entry.get("mp4") or dataset_entry.get("video")
            target_filename = targets[current_key]

            if video_bytes:
                local_output_path = save_video_bytes(target_filename, video_bytes)
                found_paths[target_filename] = local_output_path
                print(f"Saved: {local_output_path} (Index: {i})")
            else:
                print(f"Found key {current_key}, but no video bytes present.")

            remaining_keys.remove(current_key)
            if not remaining_keys:
                break

            if i % 1000 == 0:
                found_count = len(targets) - len(remaining_keys)
                print(f"... checked {i} videos, found {found_count}/{len(targets)} ...")

            if max_search is not None and i >= max_search:
                break

        if remaining_keys:
            print(f"Not found in subset {data_dir}: {len(remaining_keys)} target(s)")
        return found_paths

    except Exception as e:
        print(f"An error occurred: {e}")
        return {}


def select_project_folder():
    root = os.getcwd()
    folders = sorted(
        name
        for name in os.listdir(root)
        if os.path.isdir(os.path.join(root, name))
    )
    if not folders:
        raise RuntimeError("No folders found in the project directory.")

    print("Select a folder:")
    for idx, name in enumerate(folders, start=1):
        print(f"{idx}. {name}")

    while True:
        choice = input("Enter number: ").strip()
        if not choice.isdigit():
            print("Please enter a valid number.")
            continue
        index = int(choice)
        if 1 <= index <= len(folders):
            return folders[index - 1]
        print("Choice out of range.")


def list_json_files(folder):
    return sorted(
        name
        for name in os.listdir(folder)
        if name.lower().endswith(".json") and os.path.isfile(os.path.join(folder, name))
    )


def load_json_entries(json_path):
    entries = []
    try:
        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        if isinstance(data, list):
            entries.extend(data)
        else:
            print("Skipping: JSON root is not a list.")
    except Exception as exc:
        print(f"Skipping: {exc}")
    return entries


def extract_query_text(entry):
    conversations = entry.get("conversations") or []
    if isinstance(conversations, list):
        for item in conversations:
            if not isinstance(item, dict):
                continue
            if item.get("from") == "human" and item.get("value"):
                return str(item["value"]).strip()
    return ""


def set_pipeline_context(video_path, query_text, frame_dir, audio_out):
    config.VIDEO_INPUT = video_path
    config.USER_DESCRIPTION = query_text or config.USER_DESCRIPTION
    config.FRAME_DIR = frame_dir
    config.AUDIO_OUTPUT = audio_out

    ingest_main.VIDEO_INPUT = video_path
    ingest_main.USER_DESCRIPTION = query_text or ingest_main.USER_DESCRIPTION
    audio_processing.VIDEO_INPUT = video_path
    audio_processing.AUDIO_OUTPUT = audio_out
    video_processing.VIDEO_INPUT = video_path
    video_processing.FRAME_DIR = frame_dir


def main_cli():
    folder = os.path.normpath(JSON_INPUT_FOLDER)
    if not os.path.isdir(folder):
        raise RuntimeError(f"Configured JSON folder not found: {folder}")
    json_files = list_json_files(folder)
    if not json_files:
        raise RuntimeError(f"No JSON files found in configured folder: {folder}")

    global_index = 1
    for json_name in json_files:
        json_path = os.path.join(folder, json_name)
        entries = load_json_entries(json_path)
        if not entries:
            continue
        json_stem = os.path.splitext(os.path.basename(json_name))[0]
        downloaded_paths = download_subset_videos(json_stem, entries)
        for entry in entries:
            filename = get_entry_target_filename(entry)
            if not filename:
                print(f"[{global_index}] Skipping: no image/video field.")
                global_index += 1
                continue
            video_path = downloaded_paths.get(filename)
            if not video_path:
                global_index += 1
                continue

            video_stem = os.path.splitext(os.path.basename(video_path))[0]
            frame_dir = os.path.join("data", "frames", "hf_ingest", json_stem, video_stem)
            audio_out = os.path.join("data", "audio", "hf_ingest", f"{json_stem}_{video_stem}.mp3")
            os.makedirs(frame_dir, exist_ok=True)
            os.makedirs(os.path.dirname(audio_out), exist_ok=True)

            query_text = extract_query_text(entry)
            set_pipeline_context(video_path, query_text, frame_dir, audio_out)
            print(f"\n[{global_index}] Ingesting {video_stem}")
            ingest_main.run_ingestion_pipeline()
            global_index += 1


if __name__ == "__main__":
    main_cli()
